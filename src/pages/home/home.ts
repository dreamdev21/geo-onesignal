...
export class HomePage {
  ...
  sendNotificationWithActionButtonsAndDeepLink() {
    window["plugins"].OneSignal.getIds(function(ids) {
      var notificationObj = {
        contents: { en: "Message with Action Buttons and Deep link" },
        include_player_ids: [ids.userId],
        data: { data_key: "data_value", openURL: "https://imgur.com/" },

        buttons: [
          { id: "id1", text: "Deep Link with URL", icon: "ic_menu_share" },
          { id: "id2", text: "just button2", icon: "ic_menu_send" }
        ]
      };
